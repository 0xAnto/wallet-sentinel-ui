...
export default function Dashboard() {
  ...
  const loadData = async (userId: string) => {
    try {
      ...
      toast({
        title: "Error",
        description:
          "Failed to load wallet data. Please check your database connection.",
        variant: "destructive",
      });
    }
  };
  ...
  const fetchBalance = async (walletId: string, address: string) => {
    setWallets((prev) =>
      prev.map((w) => (w.id === walletId ? { ...w, isLoading: true } : w))
    );
    ...
    try {
      ...
      setWallets((prev) =>
        prev.map((w) =>
          w.id === walletId ? { ...w, balance, isLoading: false } : w
        )
      );
    } catch (error) {
      console.error("Error fetching balance:", error);
      setWallets((prev) =>
        prev.map((w) => (w.id === walletId ? { ...w, isLoading: false } : w))
      );
    }
  };
  ...
  const handleAddWallet = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description:
          "Failed to add wallet. Please check your database connection.",
        variant: "destructive",
      });
    }
  };
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400">
              Monitor your Aptos wallets and stay alert
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  value={newThreshold}
                  onChange={(e) =>
                    setNewThreshold(Number.parseFloat(e.target.value) || 10)
                  }
                  className="bg-gradient-to-r from-purple-100/60 to-blue-100/60 dark:from-purple-900/40 dark:to-blue-900/40 border-purple-300/40 dark:border-purple-600/40 backdrop-blur-sm focus:border-purple-500/60 dark:focus:border-purple-400/60"
                />
              </div>
          ...
          {filteredAndSortedWallets.map((wallet, index) => {
            const status =
              wallet.balance !== undefined
                ? getBalanceStatus(wallet.balance, wallet.threshold)
                : null;
    ...
    `;
              ...
              <div className="w-full">
                ...
                <div className="block md:hidden p-3 space-y-3">
                  ...
                  <div>
                    ...
                    <div
                      className={`font-mono ${inputClasses} p-2 rounded border text-xs break-all`}
                    >
                      {`${wallet.address.slice(0, 12)}...${wallet.address.slice(
                        -8
                      )}`}
                    </div>
                  </div>
                ...
                <div className="hidden md:flex items-center w-full p-4 gap-4 border-b">
                  ...
                  <div className="flex-1 min-w-0">
                    <div
                      className={`font-mono ${inputClasses} p-2 rounded border truncate`}
                    >
                      {wallet.address}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <Input
                      ...
                      onChange={(e) =>
                        handleUpdateThreshold(
                          wallet.id,
                          Number.parseFloat(e.target.value) || wallet.threshold
                        )
                      }
            ...
            const renderGridMode = () => (
              <>
                <CardHeader
                  className={`${
                    viewMode === "small-grid" ? "pb-2 px-3 pt-3" : "pb-3"
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <CardTitle
                ...
                >
                  ...
                  <div
                    className={viewMode === "small-grid" ? "block" : "block"}
                  >
                    <Label className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">
                      Alert Threshold
                    </Label>
        ...
        {wallets.length === 0 && (
          <GradientCard className="backdrop-blur-md bg-gradient-to-br from-purple-50/90 to-blue-50/90 dark:from-purple-900/20 dark:to-blue-900/20 border border-purple-200/30 dark:border-purple-700/30">
            <CardContent className="text-center py-12">
              ...
              <p className="text-gray-500 dark:text-gray-500">
                Add your first wallet to start monitoring balances and receiving
                alerts.
              </p>
            </CardContent>
          </GradientCard>
